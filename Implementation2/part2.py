...
class LogisticRegression:
    ...
    def gradient_descent(self, x, y, learning_rate, ld, max_iters, N, M):
        ...
        for i in range(max_iters):
            ...
            # L1 norm contribution (Regularization)
            w[1:] = np.sign(w[1:]) * np.maximum(abs(w[1:]) - learning_rate * ld, 0)
    ...
    def cal_loss(self, h, y, w, ld):
        n = h.shape[0]
        return np.sum((-y * np.log(h) - (1 - y) * np.log(1 - h))) / n + ld * np.linalg.norm(w[1:], ord=1)

    def predict(self, x, w):
        return np.round(self.sigmoid(np.dot(x, w)))
...
with open('w_mat_l1.csv', 'w', newline='') as f:
    write = csv.writer(f)
    write.writerow(col_names)
    write.writerows(w_mat)

with open('accuracy_l1.csv', 'w', newline='') as f:
    write = csv.writer(f)
...
with open('accuracy_dev_l1.csv', 'w', newline='') as f:
    write = csv.writer(f)
